						...
						<div class="row">
							...
							<div class="tabs">
                                ...
                                @foreach($genres as $id => $genre)
									<li class="nav-item "><a class="nav-link genre-link" data-toggle="tab" href="#" id="featured-tab" data-genre-id="{{ $genre->GenreID }}"><div class="tab-title">{{ $genre->GenreName }}</div></a></li>
								@endforeach
									<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#" id="bestseller-tab"><div class="tab-title">Other Languages</div></a></li>
								</ul>
							</div>
							<div class="tab-content float-left w-100">
								<div class="tab-pane active float-left w-100" id="#ttdictionary-main" role="tabpanel" aria-labelledby="dictionary-tab">
									<section id="ttfeatured" class="ttfeatured-products">
										<div class="ttfeatured-content products grid owl-carousel displayProducts" id="owl1">
                                            <div class="product-layouts">
                                                <div class="product-thumb">
                                                    <div class="image zoom">
...
@section('scripts')
        <script>
            // Lắng nghe sự kiện click trên một phần tử cha tồn tại từ trước (ví dụ: document)
            document.addEventListener('click', function(e) {
                // Kiểm tra xem sự kiện click có xuất phát từ một liên kết thể loại không
                if (e.target.classList.contains('genre-link')) {
                    e.preventDefault(); // Ngừng chuyển hướng trang

                    var genreID = e.target.getAttribute('data-genre-id'); // Lấy ID của thể loại từ thuộc tính data
            ...
            `).join('');
                            ...
                            // Gắn sự kiện lắng nghe cho các phần tử mới tạo
                            var el = document.querySelectorAll('.product-layouts');
